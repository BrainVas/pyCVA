...
class DSAPhaseClassifier:
    def __init__(self, num_classes=4, img_size=224):
        ...
        model_path = os.path.join(MODELS_DIR, "multi_frame_best.model")
        downloadIfNeeded.download("https://zenodo.org/records/15635651/files/multi_frame_best.model?download=1", model_path)      
...
if __name__ == '__main__':
    ...
    artery_start, artery_end, parenchymal_end, phases = output

    with open("output/phase_result.json", "w") as f:
        json.dump({
            "artery_start": artery_start,
            "artery_end": artery_end,
            "parenchymal_end": parenchymal_end,
            "phases": phases
        }, f, indent=2)
