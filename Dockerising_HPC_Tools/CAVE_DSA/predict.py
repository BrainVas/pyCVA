...
# def convert_to_binary_image(img_array):
#     _, binary_img = cv2.threshold(img_array, 0, 255, cv2.THRESH_BINARY)
...
def download_files(url_list, folder):
    ...
    if not os.listdir(folder):
        ...
        print(f"Model folder already populated at {folder}")
...
def convert_to_binary_image(img_array):
    ...
    binary_img = pil_img.point(lambda p: 255 if p > 0 else 0)

    return np.array(binary_img).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    urls = [
        "https://zenodo.org/records/15641964/files/minip_av_sigmoid_image1024_valiant-resonance-1199.pt?download=1",
        "https://zenodo.org/records/15641964/files/minip_av_sigmoid_image512_charmed-cosmos-1086.pt?download=1",
        "https://zenodo.org/records/15641964/files/minip_vessel_sigmoid_image1024_solar-resonance-1172.pt?download=1",
        "https://zenodo.org/records/15641964/files/minip_vessel_sigmoid_image512_polished-lake-1078.pt?download=1",
        "https://zenodo.org/records/15641964/files/sequence_av_sigmoid_image512_ConvGRU_logical-star-1097.pt?download=1",
        "https://zenodo.org/records/15641964/files/sequence_vessel_sigmoid_image512_ConvGRU_devoted-pine-1024.pt?download=1"
        # add all files here
    ]

    download_files(urls, MODELS_DIR)
    ...
    if os.path.isfile(args.in_img_path):
        ...
        if args.label_type == 'av':
            ...
            out_artery_bin_path = args.out_img_path.replace('.png', '_bin_artery.png')

            out_vein_img_path = args.out_img_path.replace('.png', '_vein.png')
            out_vein_bin_path = args.out_img_path.replace('.png', '_bin_vein.png')

            Image.fromarray(out_seg[0]).save(out_artery_img_path)
            Image.fromarray(convert_to_binary_image(out_seg[0])).save(out_artery_bin_path)

            Image.fromarray(out_seg[1]).save(out_vein_img_path)
            Image.fromarray(convert_to_binary_image(out_seg[1])).save(out_vein_bin_path)
        else:
            out_bin_img_path = args.out_img_path.replace('.png', '_bin.png')
            Image.fromarray(out_seg[0]).save(args.out_img_path)
            Image.fromarray(convert_to_binary_image(out_seg[0])).save(out_bin_img_path)
